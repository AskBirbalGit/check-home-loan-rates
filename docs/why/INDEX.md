...
| lib/rate-engine.ts | [journal](journal/), [0006](decisions/0006-mirror-based-rate-inheritance.md) | Typed RateEngine: 10x-granular averaged rate floored to nearest 0.05%, with 600-649 derived from the `<700` high endpoint via PSB/PVT/SFB/HFC uplifts; plus `mirror`/`resolveBands` inheritance for long-tail lenders; mirror targets bucketed by observed public rate ranges (docs/rate-mirror-mapping.md); Union Bank of India uses Canara Bank's exact bands; Bajaj Housing Finance `<700` SE is `9.00–11.00`; `others()` excludes mirrored lenders from suggestions, then rate-sorts with PSB/PSU, PVT, then other type tie-breaks |
