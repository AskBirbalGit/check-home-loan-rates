...
| RateEngine.rateFor | [journal](journal/), [0006](decisions/0006-mirror-based-rate-inheritance.md) | 10x-granular averaged rate within 50-pt CIBIL bands, floored to nearest 0.05%; 600-649 derives from the `<700` cells with lender-type uplift; follows mirrored lenders to their closest peer's bands |
...
| data/cibil-roi-rates.csv | [journal](journal/) | Source ROI sheet copied into the repo; Union Bank of India row now matches Canara Bank's rate bands |
| cibil/600-649 | [journal](journal/) | New lowest CIBIL band derived from `<700` rates with type uplifts: PSB/PVT +0.50%, SFB +0.75%, HFC +1.00% |
...
| lib/rate-engine.ts | [journal](journal/), [0006](decisions/0006-mirror-based-rate-inheritance.md) | Typed RateEngine: 10x-granular averaged rate floored to nearest 0.05%, with 600-649 derived from the `<700` cells via PSB/PVT/SFB/HFC uplifts; plus `mirror`/`resolveBands` inheritance for long-tail lenders; mirror targets bucketed by observed public rate ranges (docs/rate-mirror-mapping.md); Union Bank of India uses Canara Bank's exact bands; `others()` excludes mirrored lenders from suggestions, then rate-sorts with PSB/PSU, PVT, then other type tie-breaks |
