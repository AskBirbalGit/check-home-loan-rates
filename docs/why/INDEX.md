...
| RateEngine.rateFor | [journal](journal/), [0006](decisions/0006-mirror-based-rate-inheritance.md) | 10x-granular averaged rate within 50-pt CIBIL bands, floored to nearest 0.05%; 600-649 derives from the high endpoint of the `<700` cell plus lender-type uplift; follows mirrored lenders to their closest peer's bands |
...
| cibil/600-649 | [journal](journal/) | New lowest CIBIL band derived from the high endpoint of `<700` rates with type uplifts: PSB/PVT +0.50%, SFB +0.75%, HFC +1.00% |
...
| scripts/generate-ui-rate-dataset.mjs | [journal](journal/) | Node + `agent-browser` CLI automation that boots `next dev`, drives Box 1 inputs and scrapes Box 2 rates for 32 first-party institutions × 5 ±15-randomized CIBIL bands × Salaried/SE |
| data/ui-rate-dataset.csv | [journal](journal/) | Generated 320-row UI-scraped dataset: per institution/band/employment, the current rate plus the three better-priced lenders the UI surfaces |
## Decisions
